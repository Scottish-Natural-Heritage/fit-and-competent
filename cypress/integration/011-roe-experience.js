describe('Roe Experience page directly', function () {
  it('should prevent access', function () {
    cy.visit('/roe-experience', {failOnStatusCode: false});
    cy.get('h1').should('contain', 'there is a problem with the service');
  });
});

describe('Roe Experience page ', function () {
  ...
  it('should allow access if the user visits all the pages in order', function () {
    cy.visit('/roe-experience');
    cy.get('h1').should('contain', 'Do you control roe deer');
  });

  it('blank entry + main button should navigate to same page with error', function () {
    cy.visit('/roe-experience');
    cy.get('#main-content form button.naturescot-forward-button').click();
    cy.url().should('include', '/roe-experience');
  ...
  it('"yes" radio + main button should display form', function () {
    ...
    cy.get('#main-content form input#roe-experience-years').click();
    cy.get('#main-content form input#roe-control-number').click();
  });
  ...
  it('"yes" radio + main button should display form and try to submit empty form but get errors', function () {
    ...
    cy.get('.govuk-error-summary ul li a')
      .should('contain', 'You must enter the number of years experience you have controlling roe deer')
      .and('contain', 'You must enter the number of roe deer you have controlled in the last 5 years');
  });

  it('"yes" radio + main button should display form and try to submit with invalid chars in experience but get errors', function () {
    cy.visit('/roe-experience');
    cy.get('#main-content form input[type="radio"][value="yes"]').click();
    cy.get('input[type="text"]#roe-experience-years').type('ab');
    cy.get('#main-content form button.naturescot-forward-button').click();
    cy.url().should('include', '/roe-experience');
    ...
    cy.get('.govuk-error-summary ul li a').should(
      'contain',
      'The number of years experience you have controlling roe deer must only contain numbers'
    );
  });
  ...
  it('"yes" radio + main button should display form and try to submit with invalid chars in experience and control but get errors', function () {
    ...
    cy.get('#main-content form input[type="radio"][value="yes"]').click();
    cy.get('input[type="text"]#roe-experience-years').type('ab');
    cy.get('input[type="text"]#roe-control-number').type('ab');
    cy.get('#main-content form button.naturescot-forward-button').click();
    cy.url().should('include', '/roe-experience');

    cy.get('h2#error-summary-title').should('contain', 'There is a problem');
    cy.get('.govuk-error-summary ul li a')
      .should('contain', 'The number of years experience you have controlling roe deer must only contain numbers')
      .and('contain', 'The number of roe deer you control must only contain numbers');
  });
});
